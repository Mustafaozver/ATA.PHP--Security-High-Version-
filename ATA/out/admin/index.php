...
if (isset($_POST["username"]) && isset($_POST["password"])) {
 ...
 if ($username == $Cusername) {
  if ($password == $Cpassword) {
   ...
   $session_msg = "Password incorrect";
   if (isset($_COOKIE[session_name()])) setcookie(session_name(),'',time()-42000,'/');
   session_destroy();
   $session_statu = false;
  }
 } else {
  $session_msg = "User name incorrect";
  if (isset($_COOKIE[session_name()])) setcookie(session_name(),'',time()-42000,'/');
  session_destroy();
  $session_statu = false;
 }
 ...
 session_name('session_ta_'.$CID) ;
 session_start();
 if (isset($_SESSION["username"]) && isset($_SESSION["password"]) && isset($_SESSION["CID"])) {
  if ($_SESSION["username"] == $Cusername) {
   if ($_SESSION["password"] == $Cpassword && $_SESSION["CID"] == $CID) {
    $session_statu = true;
   } else {
    $session_msg = "Password changed";
    if (isset($_COOKIE[session_name()])) setcookie(session_name(),'',time()-42000,'/');
    session_destroy();
    $session_statu = false;
   }
  } else {
   $session_msg = "User is not found";
   if (isset($_COOKIE[session_name()])) setcookie(session_name(),'',time()-42000,'/');
   session_destroy();
   $session_statu = false;
  }
...
// Control Panel
 ...
 // $sayfa = explode("/",substr(strtolower($_SERVER["REQUEST_URI"]),1));
 $eklentiler = "_eklenti/";
 $sekmeler = "";
 $icerik = "";
 $sekme_s = 0;
...
?>
    ...
    <header class="header">
	  ...
	  <a href="index.html" class="navbar-brand font-weight-bold text-uppercase text-base">FikiTech Admin Panel</a>
        <ul class="ml-auto d-flex align-items-center list-unstyled mb-0">
          <li class="nav-item">
            <form method="post" action="" id="searchForm" class="ml-auto d-none d-lg-block">
              <div class="form-group position-relative mb-0">
				<input type="hidden" name="action" value="search" />
                <button type="submit" style="top: -3px; left: 0;" class="position-absolute bg-white border-0 p-0"><i class="o-search-magnify-1 text-gray text-lg"></i></button>
                <input name="search" type="search" placeholder="Search ..." class="form-control form-control-sm border-0 no-shadow pl-4">
              </div>
            </form>
          </li>
          ...
          <li class="nav-item dropdown ml-auto"><a id="userInfo" href="http://example.com" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="nav-link dropdown-toggle"><img src="img/logo.png" alt="Jason Doe" style="max-width: 2.5rem;" class="img-fluid rounded-circle shadow"></a>
    ...
    <section><?php echo $sekmeler; ?></section>

	  
      <div class="page-holder d-flex flex-wrap">
	  <section><?php echo $icerik; ?></section>
	 <!-- <section>
      ...
      <div class="container">
        <div class="row align-items-center py-5">
          ...
          <div class="col-lg-5 px-lg-4">
            <h1 class="text-base text-primary text-uppercase mb-4">FikiTech Control Panel Login</h1>
            <h2 class="mb-4">...</h2>
            <p class="text-muted">T:<?php echo $session_msg; ?></p>
			
			
			
			
            <form method="post" id="loginForm" action="" class="mt-4">
              <div class="form-group mb-4">
                <input type="text" name="username" value="<?php echo "".@$_POST["username"]; ?>" placeholder="Username or Email address" class="form-control border-0 shadow form-control-lg">
              </div>
              <div class="form-group mb-4">
...
?>
	...
	<!-- main page --><script src="js/charts-home.js"></script>
